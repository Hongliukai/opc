...
func (es *emptyServer) SyncCache()                      {}
func (es *emptyServer) RemoveCache([]string)            {}
...
func (oms *OpcMockServerStatic) ReadItems(s ...string) map[string]Item {
	if len(s) == 0 {
		return oms.Read()
	}
	allItems := oms.Read()
	...
	for _, tag := range s {
		if v, ok := allItems[tag]; ok {
			answer[tag] = v
		} else {
			answer[tag] = Item{0.0, OPCQualityGood, time.Now()}
		}
	}
	return answer
}
...
func (oms *OpcMockServerRandom) ReadItems(s ...string) map[string]Item {
	if len(s) == 0 {
		return oms.Read()
	}
	allItems := oms.Read()
